...
description: 'How I built a "currently watching" feed for my Discord community — and why I ditched the HTTP endpoint in favor of something already running.'
pubDate: '2026-03-16T12:00:00'
tags: ['discord', 'shiroani', 'architecture']
---

I wanted an activity feed for my Discord server. When a community member watches anime in the ShiroAni desktop app, the bot posts it to a channel: "Shirone is watching Attack on Titan." Simple social feature, helps people discover shows, maybe sparks a conversation.
...
ShiroAni's Discord bot runs on NestJS. It's currently a pure Discord gateway client — connects to Discord's WebSocket, listens for events, responds to slash commands. No HTTP server, no exposed ports. To add an HTTP endpoint, I'd need to:
...
4. Expose the port through my Docker/Coolify/Traefik stack
...
But more importantly — do I actually need it?
...
Discord fires a `presenceUpdate` event every time a user's presence changes. The bot is already connected to the gateway. All I need is:

1. Enable the `GuildPresences` intent (one line)
2. Listen for `presenceUpdate` events
3. Filter for activities from my app (match application ID `1481042476402872361`)
4. Post to the configured channel

No HTTP server. No exposed port. No API key. No authentication. No new infrastructure. The data is already flowing through a channel the bot is already connected to.
...
The gateway path has one more hop (through Discord's servers), but it eliminates the entire HTTP stack from my bot. The latency difference is seconds at most — presence updates propagate through the gateway in 1-5 seconds.
...
The only advantage of HTTP is full control over the payload and instant latency. But I don't need either. The RPC presence already contains everything: anime title, episode info, cover art URL, and AniList ID. And "user started watching" isn't a time-critical event — a few seconds delay is invisible.

## Implementation

The actual event handler is short. The interesting part is the filter chain — `presenceUpdate` fires for **every** presence change for **every** guild member. That includes going online/offline, changing game activity, updating Spotify status, everything. The handler needs to be extremely lightweight for the 99% of events it doesn't care about.
...
The ShiroAni application ID check is the first meaningful filter. Most presence updates don't involve my app at all, so they bail out after checking 0-3 activities — microseconds. The more expensive checks (Redis, database) only run for users actually running ShiroAni.
...
The desktop app updates presence every 15 seconds while active. Without dedup, someone watching a 24-minute episode would generate ~96 presence updates, each triggering the event handler. Even with all the early-return filters, the handler would hit the database and potentially post duplicate messages.
...
One Redis call does both the check and the set atomically. If the key exists, `NX` makes `SET` return null and the handler skips. If it doesn't exist, the key is set with a 30-minute expiry and the message is posted.
...
## What about the HTTP endpoint I didn't build?

I might still add an HTTP adapter to the bot eventually. Health checks for Docker, a future web dashboard, maybe the anime search feature wanting to pre-warm a cache. But I won't add it *for the activity feed.*

The lesson: before adding infrastructure, check if the data is already flowing through a channel you have access to. Discord's gateway is a firehose of events. Most bots only tap a fraction of it. The presence data was already there — I just wasn't listening.
